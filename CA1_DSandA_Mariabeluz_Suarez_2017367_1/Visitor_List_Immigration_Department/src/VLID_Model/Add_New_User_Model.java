	...
	///CONNECTION TO DB FOR LOGIN CLASS
		public void db() {
			try{
				...
				}else if(controller.QueryType =="INSERT BEGIN") {
					query = "INSERT INTO `vlid`.`users` (`idUsers`, `firstName`, `LastName`, `Date_Arrival`, `Passport`, `Priority_Level`) VALUES ('1','"+controller.FirstName+"', '"+controller.LastName+"', '"+controller.DateArrival+"', '"+controller.Passport+"', '"+controller.PriorityLevel+"')";
				}else if(controller.QueryType=="UPDATE"){
					query = "UPDATE `vlid`.`users` SET `Possition` = '"+controller.Possition+"', `PreviousNode`='"+controller.PosPrevNode+"', `NextNode`=0 WHERE (`idUsers` = '"+controller.ID+"')";
				}else if(controller.QueryType=="UPDATE_MIDDLE"){
					query = "UPDATE `vlid`.`users` SET `Possition` = '"+controller.Possition+"', `PreviousNode`='"+controller.DLL.PreviousNode+"', `NextNode`='"+controller.DLL.NextNode+"' WHERE (`idUsers` = '"+controller.ID+"')";
				}else if(controller.QueryType=="UPDATE_MIDDLE_PREV"){
					query = "UPDATE `vlid`.`users` SET `NextNode`='"+controller.ID+"' WHERE (`idUsers` = '"+controller.DLL.PreviousNode+"')";
				}else if(controller.QueryType=="UPDATE_MIDDLE_NEXT"){
					query = "UPDATE `vlid`.`users` SET `PreviousNode`='"+controller.ID+"' WHERE (`idUsers` = '"+controller.DLL.NextNode+"')";
				}else if(controller.QueryType=="UPDATE_LOW"){
					query = "UPDATE `vlid`.`users` SET `NextNode`='"+controller.ID+"' WHERE (`idUsers` = '"+controller.PosPrevNode+"')";
				}
				...
				public void db_ID() {
					try{
						...
						if(controller.QueryType =="ALL") {
							query = "select * from users order by priority_level, idusers, PreviousNode asc";
						...
						while(rs.next()) {
							...
							if(controller.QueryType=="ALL") {
								...
								controller.users[i][2]=Integer.parseInt(rs.getString("PreviousNode"));
								controller.users[i][3]=Integer.parseInt(rs.getString("NextNode"));
							}else if(controller.QueryType=="ONE"){
